...
const CaptainSignup = () => {
  ...
  const submitHandler = async (e) => {
    ...
    if(response.status === 201){
      const data = response.data //in response we get captain and token
      setCaptain(data.captain) 
      localStorage.setItem('captain-token', data.token)  //named the token as captain-token
    ...
    <div>
      <div className='py-5 px-5 h-screen flex flex-col justify-between'>
        <div >
          ...
          <p className='text-center mb-5'>Already have an account? <Link to='/captain-login' className='text-blue-600 font-semibold'>Login here</Link></p>
        </div>

        <div>
            <p className='text-[10px] leading-tight pb-10'>This site is protected by reCAPTCHA and the <span className='underline'>Google Privacy
